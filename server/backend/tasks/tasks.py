...
from backend.utils.notifications import send_reminder, send_admin_monthly_report,send_user_monthly_report, send_csv
from sqlalchemy import func
from datetime import datetime, timedelta
from dotenv import load_dotenv

from backend.utils.timehelper import get_previous_month_bounds_as_utc, get_tomorrow_ist_bounds_as_utc
load_dotenv()
import logging
...
def send_daily_reminders():
    ...
    print(f"[👥] Found {len(users)} users with reservations.")
...
def send_monthly_reports():
    ...
    for user in users:
        if user.is_admin:
            send_admin_monthly_report(user, first_day, last_day)
        else:
            send_user_monthly_report(user, first_day, last_day)
